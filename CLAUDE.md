...
- **Login/session — done.** Supabase Auth is live: email/password sign in/up and a magic-link option, via `lib/auth-context.tsx` (`signIn`, `signUp`, `sendMagicLink`, `signOut`). `lib/supabase.ts` uses `createBrowserClient` from `@supabase/ssr`, so the session JWT is sent with every request. Auth guards (redirect to `/auth` if `!user`) are wired on `/setup`, `/term`, `/members`, `/runsheet`, `/runsheets`, `/events/[id]`. `/`, `/help`, `/privacy`, `/terms`, and `/childsafety` are intentionally public — do not add a guard to those.
